...
def get_search_variants(
    ...
    normalize_name_ua = normalize_city_name(city_name_ua)
    normalize_name_en = normalize_city_name(city_name_en) if city_name_en else None

    variants = [normalize_name_ua]

    if normalize_name_ua:
        variants.append(normalize_name_ua)
...
def set_ids(
    ...
    try:
        ...
        logger.error(f"Error setting id for city {city.name_ru}: {e}")
        return False
...
class BaseCityParser(ABC):
    ...
    def _filter_cities(self, cities: List[CityModel]) -> List[CityModel]:
        ...
        valid_cities = []
        for c in cities:
            name_ru = str(c.name_ru or "").lower()

            if "тест" in name_ru or "test" in name_ru:
                continue
            if "остановка на ланч" in name_ru:
                continue
...
class BulkCityParser(BaseCityParser):
    ...
    async def run(self):
        ...
        with settings.get_db() as s:
            ...
            for city in cities:
                city_ru = str(city.name_ru)

                if city_ru in self.manual_map:
                    mapped_id = self.manual_map[city_ru]
                    logger.info(
                        f"[{self.site_name}] Found manual mapping for '{city_ru}': {mapped_id}"
                    )
                ...
                if found_id:
                    logger.success(
                        f"[{self.site_name}] Found city '{city_ru}' (variant: '{used_variant}') with ID {found_id}."
                    )
                    ...
                    logger.error(
                        f"[{self.site_name}] City '{city_ru}' not found in bulk data. Checked variants: {variants}"
                    )
