...
from app.scraper.db.db_helper import update_if_changed
from app.scraper.managers.http.likebus_api import get_cities
from app.scraper.logger import logger
from app.scraper.schemas.city_schema import CityCreate, CitySchema
...
class CityCRUD:
    ...
    def prepare_city_info(city_data: dict) -> CityCreate:
        ...
        try:
            ...
            # Retrieve names with fallback to a default value
            name_ru = city_data["loc"]["ru"].get("name", "")
            name_ua = city_data["loc"]["ua"].get("name", "")
            name_en = city_data["loc"]["en"].get("name", "")

            # Validate that at least one name is non-empty
            if not any([name_ru.strip(), name_ua.strip(), name_en.strip()]):
    ...
    def update_existing_city(
        ...
        fields_to_update = ["name_ru", "name_ua", "name_en", "like_bus_id"]
        ...
        if updated:
            logger.info(
                f"Updated city {validated_city.name_ru} with LikeBus ID {validated_city.like_bus_id}."
            )
        return updated
    ...
    def add_new_city(db: Session, validated_city: CityCreate) -> CityModel:
        ...
        db.commit()
        logger.info(
            f"Added new city {validated_city.name_ru} with LikeBus ID {validated_city.like_bus_id}."
        )
        return db_city
