...
from fastapi import Depends, Response, HTTPException  # type: ignore
...
from app.backend.api.auth.schemas import TokenResponse, TwoFASetupResponse, TwoFARequiredResponse
from app.backend.api.auth.twofa import create_temp_token, generate_otp_uri, generate_otp_secret, decode_temp_token, \
    verify_otp_code
from app.backend.api.users.schemas import UserResponse
from app.backend.api.utils.auth_utils import get_payload, blacklist_token, is_token_blacklisted, create_access_token, \
    create_refresh_token
from app.backend.exceptions.auth_exceptions import AdminAccessException, InvalidCodeException, \
    AnalyticsOrAdminAccessException
from app.backend.exceptions.token_exceptions import TokenBlacklistedException, InvalidTokenException
from app.backend.exceptions.user_exceptions import UserNotFoundException
from app.core.models import User
from app.backend.settings import settings, get_db, get_db
...
def require_analytic_or_admin(current_user: User = Depends(get_current_user)) -> User:
...
def perform_login_logic(
    ...
    if user.email == settings.ADMIN_LOGIN:
        access_token = create_tokens(user.email, response)
