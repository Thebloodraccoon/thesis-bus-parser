...
const savePreset = async () => {
  ...
  try {
    ...
    const { data } = await apiClient.post('/presets/', payload);
  ...
  toast.add({ severity: 'error', summary: 'Failed', detail: errorMessage, life: 3000 });
  } finally {
    saving.value = false;
  }
};
...
const deletePresetConfirmed = async () => {
  ...
  try {
    await apiClient.delete(`/presets/${selectedPreset.value.id}`);
    selectedPresetIdLocal.value = null;
    emit('reset');
    await fetchPresets();
    showDeleteDialog.value = false;
  } catch (err) {
  const errorMessage = err.response?.data?.detail || 'Failed to delete template';
  toast.add({ severity: 'error', summary: 'Failed', detail: errorMessage, life: 3000 });
  } finally {
...
<template>
  <div class="mb-4" style="display: flex; flex-direction: row; align-items: flex-start; gap: 0.75rem;">
    <div style="display: flex; flex-direction: column;">
      <label for="presetDropdown" class="text-sm font-medium mb-1">Filter templates</label>
      <Dropdown
        :disabled="props.loading"
        id="presetDropdown"
        v-model="selectedPresetIdLocal"
        :options="[{ id: null, name: '➕ New template' }, ...presets.map(p => ({ id: p.id, name: p.name }))]"
        optionLabel="name"
        optionValue="id"
      ...
      >
        <template #value="slotProps">
          <span v-if="slotProps.value === null">➕ New template</span>
          <span v-else>{{ presets.find(p => p.id === slotProps.value)?.name || '➕ New template' }}</span>
        </template>
      </Dropdown>
    </div>
    ...
    <div class="flex gap-2" style="margin-top: 1.625rem;">
       ...
       :disabled="props.loading"
        ...
        v-tooltip.top="'Save as new template'"
      ...
      <Button
        ...
        v-tooltip.top="'Delete the selected template'"
...
>
  <div class="mb-4">
    <label for="presetName" class="block text-sm font-medium mb-1">Template Name</label>
    <InputText
      id="presetName"
  ...
  <template #footer>
    <Button label="Cancel" icon="pi pi-times" class="p-button-text" @click="showSaveDialog = false" />
...
>
  <p class="mb-4">
You definitely want to remove the template:
    <strong>{{ selectedPreset?.name || 'Untitled' }}</strong>?
  </p>

  <template #footer>
    <Button label="Cancel" icon="pi pi-times" class="p-button-text" @click="showDeleteDialog = false" />
    <Button label="Confirm" icon="pi pi-check" class="p-button-danger"
        :loading="deleting"
        :disabled="deleting"
