...
from sqlalchemy import select, func, literal_column, and_  # type: ignore
from sqlalchemy.orm import Session, aliased  # type: ignore

from app.scraper.models import RouteModel, TripModel, SiteModel, CityModel
from app.scraper.routes.schema import RouteSchema, TripSchemaResponse
from app.scraper.routes.utils.data_processing import (
    create_route_entry,
    process_trips_result,
    process_routes_result,
    initialize_route_by_cities_result,
    add_route_for_date,
)
from app.scraper.routes.utils.query_builders import (
    get_available_sites,
    build_latest_history_subquery,
    get_filtered_trip_subquery,
    build_carrier_condition,
...
class RouteCRUD:
    ...
    def get_trips_data_by_route(
        ...
        trips_query = (
            ...
            .join(latest_history, literal_column("true"), isouter=True)
    ...
    def _get_unique_routes(
        ...
        unique_routes_query = (
            select(
                from_city.name_ru.label("from_city_name"),
                to_city.name_ru.label("to_city_name"),
            ...
            .order_by(from_city.name_ru, to_city.name_ru)
        )
    ...
    def _get_routes_data(
        ...
        carrier_condition = build_carrier_condition()
        ...
        sites_query = (
            select(
                ...
                func.min(latest_history.c.price).filter(carrier_condition).label("our_min_price"),
                func.max(latest_history.c.price).filter(carrier_condition).label("our_max_price"),
                func.min(latest_history.c.price).filter(~carrier_condition).label("competitor_min_price"),
                func.max(latest_history.c.price).filter(~carrier_condition).label("competitor_max_price"),
                func.count(func.distinct(TripModel.id)).filter(carrier_condition).label("our_segments_count"),
                func.count(func.distinct(TripModel.id)).filter(~carrier_condition).label("competitor_segments_count"),
                # fmt: on
            )
            .outerjoin(
                TripModel,
