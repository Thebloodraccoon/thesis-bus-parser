...
from app.exceptions.scraper_exceptions import TaskNotFound, TaskAlreadyExisting
from app.scraper.celery.schedule.schema import ScheduleCreate, ScheduleUpdate
from app.scraper.celery.schedule.utils import ScheduleCRUD
from app.scraper.celery.task.schema import TaskBase, TaskResponse
from app.scraper.celery.utils import update_celery_cache
from app.scraper.models import CeleryTasksModel
...
def build_schedule_create_or_update(
    ...
    schedule_kwargs = {
        "minute": str(task_data.execution_time.minute),
        "hour": str(task_data.execution_time.hour),
        "day_of_month": "*",
        "month_of_year": "*",
        "day_of_week": ",".join(str(day) for day in task_data.weekdays),
    }
...
class TaskCRUD:
    ...
    def _serialize_task(self, task: CeleryTasksModel) -> Optional[TaskResponse]:
        ...
        return TaskResponse(
            ...
            execution_time=f"{int(schedule.hour):02}:{int(schedule.minute):02}",
        )
    ...
    def create_task(self, task_data: TaskBase) -> Optional[TaskResponse]:
        ...
        created_task = CeleryTasksModel(
            name=task_name,
            task="app.tasks.celery_tasks.run_single_parser",
