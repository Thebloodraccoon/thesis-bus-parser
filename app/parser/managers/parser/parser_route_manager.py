...
from app.parser.db.route_db import RouteCRUD
from app.parser.db.trip_db import TripCRUD
from app.parser.db.trip_history_db import TripHistoryCRUD
from app.parser.managers.browser.browser_manager import BrowserManager
...
from app.parser.schemas.types import RouteData, ParsingResult
from app.parser.settings.conf import get_db
from app.parser.settings.exceptions import ParserException
from app.parser.settings.logger import get_logger, handle_processing_error
    ...
    async def process_single_route(
        ...
        try:
            ...
            if browser_manager:
                ...
                if not page:
                    raise ParserException("Failed to create browser page")

            content = await self._fetch_content(selected_date, route, page)
            ...
            if content:
                ...
                if parsed_data:
                    ...
                    logger.warning(f"Failed to parse {route} because zero tickets")
            ...
            if page and browser_manager:
                await browser_manager.close_page(page)

    async def _fetch_content(self, date, route, page):
        if page:
            return await self.parser_class.get_content(
                date, route.departure_city, route.arrival_city, page=page
            )
